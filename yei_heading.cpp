...
/* ----------------------------------------------------------------------------
	...
	Notes: -Uses the Serial class in serial.cpp, refer for more details about
				the serial comms.
   ------------------------------------------------------------------------*/
Yei_heading::Yei_heading(std::string port_name, unsigned int baud){
	updateRate = 0;
	stopIssued = 0;
...
/* ----------------------------------------------------------------------------
	...
	Returns: Nothing
   ------------------------------------------------------------------------*/
void Yei_heading::printSerialNumber(){
	/*Send the command:
	0xF7 - Binary bytes
...
/* ----------------------------------------------------------------------------
	...
	Returns: Nothing
   ------------------------------------------------------------------------*/
void Yei_heading::start(uint32_t updateRate, uint32_t streamDuration, uint32_t initDelay){
	//save the rate
	...
	/*set the filter mode*/
	std::vector<uint8_t> modeCommand;
	modeCommand.push_back(0xF7); //Binary packet byte
	modeCommand.push_back(0x7B); //Command byte
	//command data
	modeCommand.push_back(0x01); //Filter mode 0x01 kalman, 0x02 Q-COMP.
	modeCommand.push_back(checksum(modeCommand)); //Checksum byte
	//Send command
	serial->send(&modeCommand.front(),modeCommand.size());	
	...
	//command data
	streamCommand.push_back(0x01); //get filtered euler angles
	...
	int result = pthread_create(&yeiThread, 0, &Yei_heading::callUpdate, this);
	if (result != 0){
...
/* ----------------------------------------------------------------------------
	...
	Returns: Nothing
   ------------------------------------------------------------------------*/
bool Yei_heading::getData(uint32_t &timeStamp, std::vector<float> &data){
	bool gotData = false;
	//lock 
...
/* ----------------------------------------------------------------------------
	...
	Returns: Nothing
   ------------------------------------------------------------------------*/
void Yei_heading::setData(uint32_t timeStamp, std::vector<float> data){
...
/* ----------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------*/
uint8_t Yei_heading::checksum(std::vector<uint8_t> data){
...
/* --------------------------------------------------------------------------
	Function: int Yei_heading::getStop(void)
...
/* --------------------------------------------------------------------------
	Function: void Yei_heading::setStop(int val)
	...
	Returns: Nothing
   ------------------------------------------------------------------------*/  
void Yei_heading::setStop(int val) {
	pthread_mutex_lock(&stopMutex);
	stopIssued = val;
	pthread_mutex_unlock(&stopMutex);
}
...
/* --------------------------------------------------------------------------
	Function: int Yei_heading::getQueueSize()
	
	Description: Method to determine how large the buffer is
	Returns: size of the buffer
   ------------------------------------------------------------------------*/  
int Yei_heading::getQueueSize() {
