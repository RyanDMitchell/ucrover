...
Filer::Filer(std::string yei1Port, std::string yei2Port, std::string lidarPort, std::string encoderPort){
	...
	if(useYei2) yei2 = new Yei(yei2Port,115200);
	...
	/*Timing values for reference
		...
		0x00000DF3 == 3571uS/~280Hz -overflows w/laser on xbee		  
		0x00000C35 == 3125uS/320Hz  -overflows w/laser on xbee
		0x000009C4 == 2500uS/400Hz  -overflows w/laser on xbee
		0x000007D0 == 2000uS/500Hz  
		(Update rate, duration, delay)*/
		
	if(useYei1) yei1->start(0x00000200,0xFFFFFFFF,0x00000000);
	if(useYei2) yei2->start(0x000007D0,0xFFFFFFFF,0x00000000);
}
...
void* Filer::file(void){
	...
	//The sensor data variables
	std::vector<float> yei1Data;
	for(unsigned int i; i < 9;i++){
		yei1Data.push_back(0.0);
	}
	uint32_t yei1TimeStamp = 0;
	
	std::vector<float> yei2Data;
	for(unsigned int i; i < 9;i++){
		yei2Data.push_back(0.0);
	}
	uint32_t yei2TimeStamp = 0;

	uint16_t laserDist = 0;
	uint32_t laserTimeStamp = 0;

	uint32_t enTimeStamp = 0;
	int32_t enLeftCount = 0;
	int32_t enRightCount = 0;

	std::vector<uint8_t> quality;
	std::vector<float> angle;
	std::vector<float> distance;
	uint32_t lidarTimeStamp = 0;
	...
	if(useYei1){
		...
		//reverse lots of memory space
		IMU1Data.reserve(500000);
		IMU1TimeStampData.reserve(500000);
		IMU1SensorData.reserve(500000);
	}
	if(useYei2){
		std::string IMU2Title ("at mS,ax g,ay g,az g,gx rad/s,gy rad/s,gz rad/s, mx gauss, my gauss, mz gauss\n");
	...
	while(getStop() == 0){
		...
		//Note: Avoid spamming the queueSize() checker or isready() checkers
 		if(useYei1){
			if(yei1->isReady()){
				yei1Data.clear();
				yei1->getData(yei1TimeStamp, yei1Data);
				yei1TimeStamp = yei1TimeStamp - startTime;
		...
		if(useYei2){
			if(yei2->isReady()){
				yei2Data.clear();
				yei2->getData(yei2TimeStamp, yei2Data);
				yei2TimeStamp = yei2TimeStamp - startTime;
		...
		if(useEncoder){
			if(encoder->isReady()){
				encoder->getData(enTimeStamp, enLeftCount, enRightCount);
				enTimeStamp = enTimeStamp - startTime;

				//copy the encoder data as a string into memory 
				char buff[250];		
				sprintf(buff,"%i,%i,%i\n", 
							 enTimeStamp, enLeftCount, enRightCount);
		...
		if(useLaser){
			if(laser->isReady()){
				laser->getData(laserTimeStamp, laserDist);
				laserTimeStamp = laserTimeStamp - startTime;
		...
		if(useLidar){
			if(lidar->isReady()){
				...
				distance.clear();
				quality.clear();
				lidar->getScanData(lidarTimeStamp, angle, distance, quality);
				lidarTimeStamp = lidarTimeStamp - startTime;
				
				//save the data for later processing
				lidarAngleData.insert(lidarAngleData.end(),angle.begin(),angle.end());
				lidarDistanceData.insert(lidarDistanceData.end(),distance.begin(),distance.end());
				lidarQualityData.insert(lidarQualityData.end(),quality.begin(),quality.end());
				//create a vector the same length as the data and fill with times
				//not v. efficient but easy to keep data alligned
				std::vector<uint32_t> tempTime (angle.size());
				std::fill (tempTime.begin(),tempTime.end(),lidarTimeStamp);
				lidarTimeData.insert(lidarTimeData.end(),tempTime.begin(),tempTime.end());
				lidarMeasurements++;
			}
		}//end uselidar
		
		//dont jam the mutexes?
		usleep(750); //for ye1(500Hz), encoder, laser
		//usleep(1500); //for all
	}//end while(getStop() == 0)
		...
		for(unsigned int i = 0; i < IMU2TimeStampData.size() ; i++){
			...
			sprintf(buff,"%i,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", 
						 IMU2TimeStampData[i], IMU2SensorData[ind+0], IMU2SensorData[ind+1], IMU2SensorData[ind+2],
										       IMU2SensorData[ind+3], IMU2SensorData[ind+4], IMU2SensorData[ind+5],
											   IMU2SensorData[ind+6], IMU2SensorData[ind+7], IMU2SensorData[ind+8]);
			std::string temp = buff;
